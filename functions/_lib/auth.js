...
export async function currentStaff(request, env) {
  ...
  return env.DB.prepare(`SELECT s.id,s.full_name,s.email,s.role,s.active FROM sessions x JOIN staff s ON s.id=x.staff_id WHERE x.token=? AND x.expires_at>datetime('now') AND s.active=1`).bind(token).first();
}
