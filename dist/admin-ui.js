(() => {
  ...
  if(cards[0]&&history){const layout=document.createElement('div');layout.className='admin-main-grid';cards[0].parentNode.insertBefore(layout,cards[0]);layout.append(cards[0],history)}
  ...
  const css=document.createElement('style');css.textContent='.admin-main-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:16px;align-items:start}.admin-main-grid>.card{margin:0}.admin-filter{display:grid;grid-template-columns:2fr 1fr;gap:10px;margin:12px 0}.admin-filter input{font:14px system-ui,sans-serif}.status{display:inline-block;border-radius:999px;padding:4px 9px;font:700 11px system-ui,sans-serif;text-transform:uppercase}.status-pending{background:#f5dfb7;color:#76521d}.status-confirmed{background:#cde6d2;color:#245b36}.status-cancelled,.status-rejected{background:#f4d1d1;color:#7d2929}@media(max-width:700px){.admin-main-grid{grid-template-columns:1fr}}@media(max-width:620px){.admin-filter{grid-template-columns:1fr}}';document.head.appendChild(css);
})();
